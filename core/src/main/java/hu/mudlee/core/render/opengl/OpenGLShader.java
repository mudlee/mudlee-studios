...
public class OpenGLShader extends Shader {
    ...
    public OpenGLShader(String vertexShaderName, String fragmentShaderName) {
        log.debug(
                "Creating shader pipeline program for vertex shader '{}' and fragment shader '{}'",
                vertexShaderName,
                fragmentShaderName);
        pipelineId = glGenProgramPipelines();
        log.debug(
                " * Pipeline was created ID:{} for {}",
                pipelineId,
    ...
    public void createUniform(int programId, String name) {
        ...
        if (location < 0) {
            ...
            log.debug(
                    "Uniform [NAME:{}] [LOCATION:{}] created for shader [ID:{}] in pipeline [ID:{}]",
                    name,
                    location,
                    programId,
                    this.pipelineId);
