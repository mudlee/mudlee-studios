...
/**
 * Low-level Vulkan buffer + device memory pair.
 * Used as a building block for vertex, index, uniform, and staging buffers.
 */
class VulkanBuffer implements Disposable {
  ...
  /**
   * Maps host-visible memory, invokes the consumer with a ByteBuffer view, then unmaps.
  ...
  /**
   * Copies contents of {@code src} into this buffer via a single-use command buffer.
   * {@code src} must be host-visible (staging), {@code this} can be device-local.
   */
