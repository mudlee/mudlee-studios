 ...
 *
 * Upload strategy: staging buffer → device-local VkImage, then two layout transitions
 * (UNDEFINED → TRANSFER_DST_OPTIMAL → SHADER_READ_ONLY_OPTIMAL) via single-use command buffers.
 *
 * STBImage is loaded with forced RGBA (4 channels) so we always use VK_FORMAT_R8G8B8A8_SRGB.
 *
 * bind() registers this texture as the active texture on VulkanContext so that
...
public class VulkanTexture2D extends Texture2D {
  ...
  private void uploadTexture(VulkanContext ctx) {
    try (MemoryStack stack = stackPush()) {
      ...
      VulkanBuffer staging = new VulkanBuffer(device, imageSizeBytes,
      ...
      transitionImageLayout(ctx.commandPool(),
  ...
  private void copyBufferToImage(VulkanBuffer buffer, int width, int height, VulkanCommandPool commandPool) {
    try (MemoryStack stack = stackPush()) {
      ...
      vkCmdCopyBufferToImage(cmdBuf, buffer.handle(), image,
  ...
  /**
   * Allocates a descriptor set from the shared pool in VulkanContext and
   * writes this texture's image view + sampler into it.
   */
